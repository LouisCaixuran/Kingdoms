...
class Board():
	...
	def toString(self):
		sit="p1:"+self.players[0].name+"\\"
		while i in  self.playerBuildings[0]:
			sit+=str(i[1])+","
		sit+="\n"
		sit+="p2:"+self.players[1].name+"\\"
		while i in  self.playerBuildings[1]:
			sit+=str(i[1])+","
		sit+="\n"
		...
		for i in range(5):
			for j in range(6):
				if self.playboard[i][j].isV==True:
					sit+=str(self.playboard[i][j].value)+","
				else:
					sit+="B"+str(self.players.index(self.playboard[i][j].owner))+str(self.playboard[i][j].level)
		return sit
